...
if [ -z "$(git status --porcelain --untracked-files=no)" ] || [ "$1" == "-f" ]; then
  ...
  autopep8 --in-place --select "$errors" -a --recursive pyms_agilent/
  autopep8 --in-place --select "$belligerent" -a -a -a -a -a --recursive pyms_agilent/
  >&2 flake8 --select "$errors$belligerent" pyms_agilent/
